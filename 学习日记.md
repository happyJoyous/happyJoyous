...
http://www.elabnote.com/

目前LQG00001  hy123456. （可以看）



当npm 下载包报错的时候，可能是因为版本不够高，更新一下版本问题
...
表示会对本地打包的文件dist进行一层预览模式，相当于线上生产，主要uniapp下打包成的h5可能要提到dist根级目录下
...
* Chinese中文简体
* Code Runner //支持编辑器执行
* CodeGeex: Ai Code AutoComplete  //自动化ai生产可以帮助写代码
...
* Tailwind CSS IntelliSense    //TailwindCss 语言高亮提示
...
如同正常用vite包装的uniapp项目模板文件

git@github.com:yang1206/uniapp-template.git

https://github.com/yang1206/uniapp-template

此为github上开源项目搭建的模板，vite构建与众多插件一起构建完成，集成了vue，vite，pinia的uniapp
...
提交
git add .
git commit -m "chore: 提交master分支"
把本地仓库连接到远程仓库
git remote add origin https://github.com/happyJoyous/happyJoyous.git
推送本地主分支
...
```
export const sortByCharCode = (_str1: string, _str2: string, direction = true): number => {
  const [str1, str2] = [_str1.trim(), _str2.trim()]
  let [preNum, afterNum] = [1, -1]
  // let compare = false
  if (!direction) {
    ;[preNum, afterNum] = [-1, 1]
  }
  if (str1 === undefined || str1 === null || str1 === ' ' || str1 === '') {
    return preNum
  }
  if (str2 === undefined || str2 === null || str2 === ' ' || str2 === '') {
    return afterNum
  }
  if (
    (str1.split('').every((char) => notChinese(char)) && str2.split('').every((char) => notChinese(char))) ||
    (str1.split('').every((char) => !notChinese(char)) && str2.split('').every((char) => !notChinese(char)))
  ) {
    return direction ? str1.localeCompare(str2, 'zh-Hans-CN') : str2.localeCompare(str1, 'zh-Hans-CN')
  } else {
    const charAry = str1.split('')
    for (const i in charAry) {
      if (charCompare(str1[i], str2[i]) !== 0) {
        return direction ? charCompare(str1[i], str2[i]) : charCompare(str2[i], str1[i])
      }
    }
  }
  return -1
  ...
  //   return afterNum
  // }
  // Array.from(str1).some((char, index) => {
  //   if (char.charCodeAt(0) - str2.charAt(index).charCodeAt(0) > 0) {
  //     compare = true
  //     return true
  //   } else if (char.charCodeAt(0) - str2.charAt(index).charCodeAt(0) < 0) {
  //     compare = false
  //     return true
  //   }
  // })

  // return compare ? preNum : afterNum
}
export function charCompare(charA, charB) {
  // 谁为非法值谁在前面
  if (charA === undefined || charA === null || charA === ' ' || charA === '') {
    return -1
  }
  if (charB === undefined || charB === null || charB === ' ' || charB === '') {
    return 1
  }
...
// 判断是否是中文
export function notChinese(chart) {
...
使用computed暴露子组件的同时，父组件得到的实例中暴露出来的变量是直接获取reactive变量，不是ComputedRef变量，也会被透析成根本变量
